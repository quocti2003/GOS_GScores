                    ...
                    <tbody>
                        ${data.map(student => `
                            <tr>
                                ...
                                <td>${(student.toan || 0) + (student.vatLi || 0) + (student.hoaHoc || 0)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
...
async function searchScores(regNumber) {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Không tìm thấy điểm cho số báo danh này');
        const data = await response.json();
        ...
        detailedScoresContent.innerHTML = '<p class="text-muted">Kết quả tìm kiếm điểm sẽ hiển thị tại đây.</p>';
    } finally {
...
if (registrationForm) {
    registrationForm.addEventListener('submit', async (event) => {
        ...
        if (!regNumber) {
            displayValidationError(registrationNumberInput, registrationNumberError, 'Vui lòng nhập số báo danh.');
            detailedScoresLoading.classList.add('d-none');
